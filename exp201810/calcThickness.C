...
//  mSi.SetZP(1., 1.);    //protons
  ...
  //   cut[i].Form("DSDY_R[%d]>100",i);
  //   cut_0[i].Form("DSDY_R[%d]<100",i);
  ...
  ch0->Add("/media/user/work/data/exp201810/calib/Left_20um/dsd_20_l_03_*");
  if (!ch0) {
    cout << "/media/user/work/data/exp201810/calib/Left_20um/dsd_20_l_03_ file was not found " << endl;
  }
...
//  myfile4.open("/home/muzalevsky/work/exp1803/data/siCal/SQY_L.cal");
  myfile1.open("/media/user/work/software/fork/useranalysis/calibration/parameters/DSDY_L.cal");
  while (! myfile1.eof() ){
    ...
    sscanf(line1.Data(),"%g %g", parYL1+count,parYL2+count);
    count++;
  }
  ...
  Double_t alpha[16];
  for(Int_t i=0;i<16;i++) {
    if(i<8) alpha[i] = atan(3.75*(7.5-i)/350.);
    if(i>7) alpha[i] = atan(3.75*(0.5+i-8)/350.);
  }
  ...
  Double_t betta[16];
  for(Int_t i=0;i<16;i++) {
    if(i<8) betta[i] = atan(3.125*(7.5-i)/344.);
    if(i>7) betta[i] = atan(3.125*(0.5+i-8)/344.);
  }
...
// //
  ...
  upFitBorder = 0.4; lowFitBorder = 0.4;
  ...
  c[0]->cd(1);
  ch0->Draw("NeEvent.DSDY_L[10]>>edepN(495,105,600)","NeEvent.DSDY_L[10]>105 && NeEvent.DSDY_L[10]<600 && NeEvent.SSD20_L[10]>100","");
...
// }
  ...
  for(Int_t i=0;i<16;i++) { // strips in SQ20 (0-15) SQX_L (0-31)
    for(Int_t nStr = 0;nStr<16;nStr++) { // strips in SQY_L (0-15)
      ...
      hDraw.Form("(NeEvent.DSDY_L[%d]*%f + %f)>>edep%d_%d(130,0.7,7)",nStr,parYL2[nStr],parYL1[nStr],i,nStr);
      //cout << hDraw.Data() << " asd asdasdasd " << parYL2[15] << endl;return; 
      hname.Form("edep%d_%d",i,nStr);
      hcut.Form("NeEvent.DSDY_L[%d]>110 && NeEvent.SSD20_L[%d]>100 ",nStr,i);
      ...
      if(nPoints/numPoints < 0.2) flag = 1;
      if(flag == 0) {
        hSpectrum = h[i];
        if (!hSpectrum) {
       ...
       // cout << "thickness!!" << endl; 
        ...
        thAv = (th[0] - 0.2 - (2.5/cos(alpha[nStr])) )/cos(betta[i]);
      } // if flag == 0
      if(flag == 1) {
...
//SetBinContent(Int_t binx, Int_t biny, Double_t content)
  ...
  // outfile
  TFile *fw = new TFile("thickness.root", "RECREATE");
  if (fw->IsOpen() == 0) {
