...
void cal_s20_3() {
  ...
  for(Int_t i=0;i<16;i++) {
    // cout << " wtf " << endl;
    c1->cd(i+1);
    hCut.Form("n20_3==%d && n3!=-1",i);
    histName.Form("h%d_summ",i);
    hDraw.Form("(a20_3.*%f) + a3>>h%d_summ(100,6,9)",oldpar2[i],i);
    ...
    hCut.Form("a20_3>50 && n20_3==%d && n3!=-1 && (a20_3.*%f) + a3 > 6.5 && (a20_3.*%f) + a3 <10 ",i,oldpar2[i],oldpar2[i]);
    histName.Form("h%d_test",i);
    hDraw.Form("%lf + (%lf -1)*a3/(a20_3.*%f) >> h%d_test",k[i],k[i],oldpar2[i],i);
  ...
  for(Int_t i=0;i<16;i++) {
    ...
    hDraw.Form("(a20_3.*%f*%lf) + a3>>h%d_summCal(100,6,9)",oldpar2[i],k1[i],i);
    ch1->Draw(hDraw.Data(),hCut.Data(),"");
  ...
  outcalfile << "2" << endl;
  outcalfile << "16" << endl;  
  for (Int_t i=0;i<16;i++) outcalfile << -p3[i]*k1[i]*oldpar2[i] << "\t" << k1[i]*oldpar2[i] << endl;
