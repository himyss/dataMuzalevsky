...
void CsIselect() {
  ...
  if(nCsI>-1 && nCsI<16 && cCsI[nCsI]->IsInside(tCsI-tF5, aCsI)) {
    flagCent = 1;  
    return;
  }
  else {
...
void SSD20_Lselect() {
  ...
  // if (n20_L>-1 && n20_L<16 && a20_L>0 && cLT20X[n20_L]->IsInside(t20_L-tF5, a20_L)) {
    return;
  }
  else {
    flagLeft = 0;
...
void SSD20_Rselect() {
  ...
  if (n20_R>-1 && n20_R<16 && a20_R>0 && cRT20X[n20_R]->IsInside(t20_R-tF5, a20_R)) {
    return;
  }
  else {
    flagRight = 0;
...
void checkHe3_R() {
  ...
  // if(nY_R>-1 && n20_R>-1 && cHelium3[n20_R]->IsInside(Y_R, a20_R)) {
    nhe3_R = 1;
    return;
  }
  else {
...
void Y_Rselect() {
  ...
  if(nY_R>-1 && nY_R<16 && Y_R>0 && cRTY[nY_R]->IsInside(tY_R-tF5, Y_R)) {
    return;
  }
  else {
    flagRight = 0;
