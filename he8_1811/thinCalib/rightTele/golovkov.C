...
void golovkov() {
  ...
  readPar("ssd20_l_pedestals",rpar1,rpar2);
  // gStyle->SetOptStat(0);

  // TChain *ch1 = new TChain("tree"); //e4
  TChain *ch1 = new TChain("tree"); //e4
  ch1->Add("/home/oem/work/data/exp1811/analysed/clb/dsd_20_l_03_selected.root");
  ...
  for(Int_t i=10;i<16;i++) {
    ...
    c6->cd(i-9);
    hCut.Form("n20_L==%d && nY_L!=-1 && a20_L_cal + Y_L_cal>6.5 && a20_L_cal + Y_L_cal<8",i);
    histName.Form("h%d_test",i);
    hDraw.Form("%lf + (%lf -1)*Y_L_cal/a20_L_cal >> h%d_test(300,0.85,1.2)",temp[i],temp[i],i);
    ch1->Draw(hDraw.Data(),hCut.Data(),"");

    hFit[i] = (TH1F*)gPad->GetPrimitive(histName.Data());
    findRange(hFit[i],&fitMax,&fitMin,0.4);
    // cout << fitMax << " " << fitMin << endl;
    fitAuto->SetRange(fitMin,fitMax);
    hFit[i]->Fit("fitAuto", "R+", "", fitMin, fitMax);
...
// return;
  ...
  ofstream outcalfile;
  outcalfile.open("/home/oem/work/software/expertroot/input/parameters/ssd20_l_new_er.cal");
  if (!outcalfile.is_open()) {
    cout << "Output file ssd20_l_new.cal was not opened" << endl;
    return;
  }//if
