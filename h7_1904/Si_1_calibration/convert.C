...
void convert() {
  ...
  inTree->Add("/home/oem/work/data/exp1904/clb/1mm_01.root");
  ...
  inTree->SetBranchAddress("NeEvent.tSSD_20u_4[16]",    tSSD_20u_4); 
    ...
    TFile *outF = new TFile("/home/oem/work/data/exp1904/clb/analysed/cut_1mm_01.root","RECREATE");
...
void fillSingleSi20(UShort_t *amp,UShort_t *time,Float_t *maxAmp,Float_t *maxTime,Int_t *nCh,Float_t *par1,Float_t *par2,Float_t *ped) {
  ...
  if(count==1 && ch>-1) {
    *maxAmp = (*(amp+ch) - *(ped+ch) )*(*(par2+ch)) + (*(par1+ch));
    // *maxAmp = (*(amp+ch))*(*(par2+ch)) + (*(par1+ch));
    *maxTime = *(time+ch);
    *nCh = ch;
  } 
