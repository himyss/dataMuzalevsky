...
void convert1() {
  ...
  ch->Add("/media/ivan/data/exp1904/digi/h7/h7_*.root");
  // ch->Add("/media/ivan/data/exp1904/digi/vetoOff/h7_ct_66_58_digi.root");
  
  // ch->Add("/home/oem/work/data/exp1906/digi/be10_ct_10_*");
    
  cout << "Found " << ch->GetEntries() << " entries" << endl;

  readPar("SSD_20u_1_calib",pSQ201_1,pSQ201_2);
  readPar("SSD_20u_2_calib",pSQ202_1,pSQ202_2);
  readPar("SSD_20u_3_calib",pSQ203_1,pSQ203_2);
  readPar("SSD_20u_4_calib",pSQ204_1,pSQ204_2);

  readPar("SSD_1m_1_MK",pSSD1_1,pSSD1_2);
  readPar("SSD_1m_2_MK",pSSD2_1,pSSD2_2);
  readPar("SSD_1m_3_MK",pSSD3_1,pSSD3_2);
  readPar("SSD_1m_4_MK",pSSD4_1,pSSD4_2);

  readPar("SSD_v_1_veto_MK",pSSD_V1_1,pSSD_V1_2);
  readPar("SSD_v_2_veto_MK",pSSD_V2_1,pSSD_V2_2);  
  readPar("SSD_v_3_veto_MK",pSSD_V3_1,pSSD_V3_2);
  readPar("SSD_v_4_veto_MK",pSSD_V4_1,pSSD_V4_2);

  readPar("DSD_X_MK",pDSD_X1,pDSD_X2,32);
  readPar("DSD_Y_MK",pDSD_Y1,pDSD_Y2,32);
...
//--------------------------------------------------------------------------------
  ...
  TFile *fw = new TFile("/media/ivan/data/exp1904/analysed/novPars/cal.root", "RECREATE");
  ...
  // for(Int_t nentry=0;nentry<100;nentry++) {    
    ...
    fillSi(v_SSD_2,SSD2,tSSD2,pSSD2_1,pSSD2_2,&mult1_2,1);
    fillSi(v_SSD_V_2,SSD_V2,tSSD_V2,pSSD_V2_1,pSSD_V2_2,&multv_2,1);

    fillSi(v_SSD20_3,SQ20_3,tSQ20_3,pSQ203_1,pSQ203_2,&mult20_3,0.5);
    fillSi(v_SSD_3,SSD3,tSSD3,pSSD3_1,pSSD3_2,&mult1_3,1);
    fillSi(v_SSD_V_3,SSD_V3,tSSD_V3,pSSD_V3_1,pSSD_V3_2,&multv_3,1);

    fillSi(v_SSD20_4,SQ20_4,tSQ20_4,pSQ204_1,pSQ204_2,&mult20_4,0.5);
    fillSi(v_SSD_4,SSD4,tSSD4,pSSD4_1,pSSD4_2,&mult1_4,1);
    fillSi(v_SSD_V_4,SSD_V4,tSSD_V4,pSSD_V4_1,pSSD_V4_2,&multv_4,1);
...
void fillarrayCsI(TClonesArray *data,Float_t* amp,Float_t* time) {
  ...
  for(Int_t i=0;i<data->GetEntriesFast();i++) {
    ...
    *(amp+nCh) = 1000*((ERQTelescopeCsIDigi*)data->At(i))->GetEdep();
...
void processCsI_cal(Float_t *par1,Float_t *par2) {
  ...
  for(Int_t i=0;i<16;i++){
    if (arCsI[i]*(*(par2+i)) + (*(par1+i))>aCsI) {
      tCsI = trCsI[i];
      nCsI = i;
      aCsI = arCsI[i]*(*(par2+nCsI)) + (*(par1+nCsI));
    }
  }

  Int_t nMax = 0;
  for(Int_t i=0;i<16;i++){
    if (arCsI[i]*(*(par2+nCsI)) + (*(par1+nCsI))==aCsI) {
      nMax++;
    }
  }
