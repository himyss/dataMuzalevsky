void drawHole(){
  ...
  //--------------------------------------------------------------------------------
    ...
    TCanvas *c_3helabCM,*c_3helabCM_diff,*c_kinTriangle_track,*c_kinTriangle_track_diff, *c_tritonAnal,*c_tritonAnal_diff;
    ...
    if (tritonAnal) {
        c_tritonAnal = new TCanvas("c_tritonAnal","triton lab CM energy correlation",1000,1000);
        c_tritonAnal->cd();
        ch1->SetMarkerColor(kBlack);
        cut.Form("nh3 && flagCent && nh3 && ( (nhe3_1 && flag1) || (nhe3_2 && flag2) || (nhe3_3 && flag3) || (nhe3_4 && flag4) )");
        cut += " && " + cutTriangle1;
        ch1->Draw("eh3_CM*1000:eh3*1000",cut.Data(),"");   
        ...
        cut += " && frame3X<13.05 && frame3X>-11.95";
        cut += " && frame3Y<13.83 && frame3Y>-11.17"; 
        ch1->Draw("eh3_CM*1000:eh3*1000",cut.Data(),"same");   

        c_tritonAnal->Print("pdf/tritonEnergy_Corr.pdf");


        c_tritonAnal_diff = new TCanvas("c_tritonAnal_diff","triton lab CM energy correlation",1000,1000);
        c_tritonAnal_diff->Divide(2,2);

        c_tritonAnal_diff->cd(2);
        ch1->SetMarkerColor(kBlack);
        ...
        cut += " && frame3Y<13.83 && frame3Y>-11.17"; 
        ch1->Draw("eh3_CM*1000:eh3*1000",cut.Data(),"same"); 
        c_tritonAnal_diff->Update();

        c_tritonAnal_diff->Print("pdf/tritonEnergy_Corr_diff.pdf");
