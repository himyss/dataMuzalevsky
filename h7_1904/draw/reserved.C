...
// return;
  ...
  if (tCsI_s) {
    ...
    for(Int_t i=0;i<16;i++) {
      ...
      // hdraw.Form("X_C.:aCsI>>h%d",i);
      // gPad->SetLogz();/
      ch->Draw(hdraw.Data(),cut.Data(),"same",5000000,0);
      // gPad->SetLogz();
      c1->Update();
  ...
  if (tritium) {
    ...
    for(Int_t i=0;i<16;i++) {
      ...
      hdraw.Form("X_C:(arCsI[%d]*%f) + %f >>h%d(500,0,150,200,0,25)",i,pCsI_2[i],pCsI_1[i],i);
      // hdraw.Form("X_C.:aCsI>>h%d",i);
      ch->SetMarkerColor(kBlack);
      ch->SetMarkerStyle(7);
      ...
      hdraw.Form("X_C:(arCsI[%d]*%f) + %f >>h%d_cut(500,0,150,200,0,25)",i,pCsI_2[i],pCsI_1[i],i);
      // hdraw.Form("X_C.:aCsI>>h%d",i);
      ch->SetMarkerColor(kRed);
      ch->SetMarkerStyle(7);
    ...
    c3h->Print("/home/ivan/Desktop/vietnam.png");
  }
  ...
  if (he3_1) {
    ...
    { 
      ...
      cut += " && nh3";
      hdraw.Form("a20_1:a1_1>>h%d_triton1(100,1,30,100,0.5,5)",i);
