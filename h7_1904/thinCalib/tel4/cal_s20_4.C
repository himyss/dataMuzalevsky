...
void cal_s20_4() {
  ...
  for(Int_t i=0;i<16;i++) {
    // cout << " wtf " << endl;
    c1->cd(i+1);
    hCut.Form("n20_4==%d && n4!=-1",i);
    histName.Form("h%d_summ",i);
    hDraw.Form("(a20_4.*%f) + a4>>h%d_summ(100,6,9)",oldpar2[i],i);
    ...
    hCut.Form("a20_4>50 && n20_4==%d && n4!=-1 && (a20_4.*%f) + a4 > 6.5 && (a20_4.*%f) + a4 <10 ",i,oldpar2[i],oldpar2[i]);
    histName.Form("h%d_test",i);
    hDraw.Form("%lf + (%lf -1)*a4/(a20_4.*%f) >> h%d_test",k[i],k[i],oldpar2[i],i);
  ...
  for(Int_t i=0;i<16;i++) {
    ...
    hDraw.Form("(a20_4.*%f*%lf) + a4>>h%d_summCal(100,6,9)",oldpar2[i],k1[i],i);
    ch1->Draw(hDraw.Data(),hCut.Data(),"");
  ...
  cout << posAv/16 << endl;
  ofstream outcalfile;
  outcalfile.open("/home/oem/work/software/AculUti/data/exp1904/ssd_20u_4.cal");
  if (!outcalfile.is_open()) {
    cout << "Output file ssd_20u_3.cal was not opened" << endl;
    return;
  }//if

  outcalfile << "2" << endl;
  outcalfile << "16" << endl;  
  for (Int_t i=0;i<16;i++) outcalfile << -p4[i]*k1[i]*oldpar2[i] << "\t" << k1[i]*oldpar2[i] << endl;
