...
//  mSi.SetZP(1., 1.);    //protons
  ...
  // TString space(" || ");
  // for(Int_t i =0;i<16;i++) {
  //   cut[i].Form("DSDY_L[%d]>100",i);
  //   cut_0[i].Form("DSDY_L[%d]<100",i);
  ...
  ch0->Add("/media/ivan/data/exp1904/clb/postClb/grouped/calib_90_all_merged.root");  
  if (!ch0) {
...
//  myfile4.open("/home/muzalevsky/work/exp1803/data/siCal/SQY_L.cal");
  myfile1.open("/home/ivan/work/soft/er/input/novPars/SSD_1m_1_cal.cal");
  while (! myfile1.eof() ){
...
return;
  ...
  for(Int_t i=0;i<16;i++) { // strips in SQ20 (0-15) SQX_L (0-31)
    for(Int_t nStr = 0;nStr<16;nStr++) { // strips in SQY_L (0-15)
      ...
      hDraw.Form("SSD_1m_1[%d]*%f + %f >>edep%d_%d(130,0.7,7)",7,i,nStr);
      hcut.Form("tSSD_20u_1[%d]>0",i);
...
//SetBinContent(Int_t binx, Int_t biny, Double_t content)
  ...
  // outfile
  TFile *fw = new TFile("/media/ivan/data/tmp/maps/thickness2.root", "RECREATE");
  if (fw->IsOpen() == 0) {
