...
void selection1() {
  ...
  ch->Add("/media/ivan/data/exp1904/analysed/novPars/calibrated/h7_1_cal.root");
  ...
  TFile *fw = new TFile("/media/ivan/data/exp1904/analysed/novPars/selected/h7_0_cut.root", "RECREATE");
...
void readThickness() {
  ...
  TFile *f1 = new TFile("/home/ivan/work/soft/er/input/exp1904_pars/MKpars/thickness1_MK.root","READ");
  if (f1->IsZombie()) {
    ...
    TH2F *hThick1 = (TH2F*)f1->Get("pseudo_Y_high_dead");
    for(Int_t i = 0; i<16; i++) {
      for(Int_t j = 0; j<16; j++) {
  ...
  TFile *f2 = new TFile("/home/ivan/work/soft/er/input/exp1904_pars/MKpars/thickness2_MK.root","READ");
  if (f2->IsZombie()) {
    for(Int_t i = 0; i<16; i++) {
      for(Int_t j = 0; j<16; j++) {
        fThickness2[i][j] = 20.;
    ...
    TH2F *hThick2 = (TH2F*)f2->Get("pseudo_Y_high_dead");
    for(Int_t i = 0; i<16; i++) {
      for(Int_t j = 0; j<16; j++) {
  ...
  TFile *f3 = new TFile("/home/ivan/work/soft/er/input/exp1904_pars/MKpars/thickness3_MK.root","READ");
  if (f3->IsZombie()) {
    for(Int_t i = 0; i<16; i++) {
      for(Int_t j = 0; j<16; j++) {
        fThickness3[i][j] = 20.;
    ...
    TH2F *hThick3 = (TH2F*)f3->Get("pseudo_Y_high_dead");
    for(Int_t i = 0; i<16; i++) {
      for(Int_t j = 0; j<16; j++) {
  ...
  TFile *f4 = new TFile("/home/ivan/work/soft/er/input/exp1904_pars/MKpars/thickness4_MK.root","READ");
  if (f4->IsZombie()) {
    for(Int_t i = 0; i<16; i++) {
      for(Int_t j = 0; j<16; j++) {
        fThickness4[i][j] = 20.;
    ...
    TH2F *hThick4 = (TH2F*)f4->Get("pseudo_Y_high_dead");
    for(Int_t i = 0; i<16; i++) {
      for(Int_t j = 0; j<16; j++) {
...
void triton() {
  if(nCsI>-1 && nX_C>-1 && cut3h[nCsI]->IsInside((aCsI-pCsI_1[nCsI])/pCsI_2[nCsI], X_C)) {
    nh3 = 1;
    return;
  }
  else {
...
void CsItimes() {
  if(nCsI>-1 && cutCsI[nCsI]->IsInside(tCsI-tF5, (aCsI-pCsI_1[nCsI])/pCsI_2[nCsI]) ) { 
    return;
  }
  else {
    flagCent = 0;  
...
void timesSQ20() {
  ...
  if (t20_1-tF5<0 || t20_1-tF5>100) flag1 = 0;

  if (t20_2-tF5<0 || t20_2-tF5>100) flag2 = 0;

  if (t20_3-tF5<0 || t20_3-tF5>100) flag3 = 0;

  if (t20_4-tF5<0 || t20_4-tF5>100) flag4 = 0;
...
void timesSQ1() {
  ...
  if (t1_4-tF5 < -100 && t1_4-tF5>100) flag4=0;
