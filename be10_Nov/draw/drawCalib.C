...
void drawCalib(){
  ...
  ch->Add("/media/ivan/data/exp1906/be10/analysed/novPars/calibrated/be10_thinTarget_new.root");
  ...
  if (centTimes) {
    ...
    for(Int_t i=0;i<16;i++) {
      ...
      cut.Form("multc_x==1 && trigger!=1",i);
      // hdraw.Form("DSD_X:aCsI>>h%d(1000,0,250,500,0,100)",i);
      hdraw.Form("DSD_X[%d]:tDSD_X[%d]-tF5 >> h%d",i+16,i+16,i);
      // ch->SetMarkerStyle(8);
      ch->Draw(hdraw.Data(),cut.Data(),"",1000000,0);
      c3h->Update();

      cutX_C[i+16]->SetLineColor(kRed);
      cutX_C[i+16]->Draw("same");
      c3h->Update();
  ...
  if (litium) {
    ...
    for(Int_t i=0;i<16;i++) {
      ...
      // hdraw.Form("DSD_X:aCsI>>h%d(1000,0,250,500,0,100)",i);
      hdraw.Form("DSD_X[]:arCsI[%d]>>h%d",i,i);
      // ch->SetMarkerStyle(8);
      ch->Draw(hdraw.Data(),cut.Data(),"",1000000,0);
      // c_arr[i]->Update();
      c3h->Update();
