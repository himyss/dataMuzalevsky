...
void convert1() {
  ...
  ch->Add("/media/ivan/data/exp1906/be10/digi/all/noVeto/secondVol/be*");
  ...
  // readPar("empty16",pSSD_V1_1,pSSD_V1_2);
  // readPar("empty16",pSSD_V2_1,pSSD_V2_2);  
  // readPar("empty16",pSSD_V3_1,pSSD_V3_2);
  // readPar("empty16",pSSD_V4_1,pSSD_V4_2);
...
//--------------------------------------------------------------------------------
  ...
  TFile *fw = new TFile("/media/ivan/data/exp1906/be10/analysed/novPars/be10_cal_2.root", "RECREATE");
  ...
  // beamdet
    if(v_F3->GetEntriesFast()!=0) fillF5(v_F5);
    if(v_F5->GetEntriesFast()!=0) fillF3(v_F3);
  
    tMWPC = ((ERBeamDetMWPCDigi*)v_MWPCx1->At(0))->GetTime();

    if (tMWPC - tF5 > 80 || tMWPC - tF5 < 63) continue;
...
void fillSi(TClonesArray *data,Float_t* amp,Float_t* time,Float_t *par1,Float_t *par2,Int_t *multiplicity, Float_t ampThreshold,Float_t timeThreshold=0) {
  ...
  for(Int_t i=0;i<data->GetEntriesFast();i++) { 
    ...
    *(time+nCh) = ((ERQTelescopeSiDigi*)data->At(i))->GetTime();
    if ( isCal && ( *(amp+nCh)<=ampThreshold || *(amp+nCh)<=timeThreshold ) ) {
      *(amp+nCh) = -10;
      *(time+nCh) = -10;
